...
class RMBTLoopModeViewController: TopLevelViewController {
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.leftBarButtonItem = UIBarButtonItem(image: UIImage(named: "close_black_24dp"), style: .plain, target: self, action: #selector(closeButtonClick(_:)))
    ...
    override func applyColorScheme() {
        ...
        currentResultView.notEmptyBackgroundColor = .clear
        ...
        medianResultView.colors = [RMBTColorManager.medianBackgroundColor]
        ...
        progressColorView.colors = [
            RMBTColorManager.progressPrimaryColor,
            RMBTColorManager.progressSecondaryColor
        ]
    ...
    func median(for identifier: String) -> String? {
        ...
        if identifier == State.qos.rawValue {
            if count > 0 {
                let percent = Double(total) * (value / Double(count))
...
//            self.navigationController?.popViewController(animated: true)
    ...
    func manageViewFinishMeasurement(uuid: String) {
        ...
        self.isResultsDownloaded = false
        self.isQosResultsDownloaded = false
        MeasurementHistory.sharedMeasurementHistory.getMeasurementDetails_Old(uuid, full: false) { [weak self] response in
            guard let self = self else { return }
            
            if let jitter = response.jitter {
                let currentValue = jitter * 1_000_000
            ...
            if let packetLoss = response.packetLoss {
                loopResult.packetLoss = String(packetLoss)
                let item = self.item(with: State.packetLose.rawValue)
                item?.currentValue = String(packetLoss)
                item?.medianValue = self.median(for: State.packetLose.rawValue)
            } else {
                loopResult.packetLoss = "-"
            }
            
            // Update values from server to make same values between app and server
            let up = Double((response.speedUpload ?? 0))
            loopResult.up = String(up)
            let uploadItem = self.item(with: State.upload.rawValue)
            uploadItem?.currentValue = loopResult.up
            uploadItem?.medianValue = self.median(for: State.upload.rawValue)
            
            let down = Double(response.speedDownload ?? 0)
            loopResult.down = String(down)
            let downloadItem = self.item(with: State.download.rawValue)
            downloadItem?.currentValue = loopResult.down
            downloadItem?.medianValue = self.median(for: State.download.rawValue)
            
            let ping = Double(response.ping ?? 0) / 1.0e-6
            loopResult.ping = String(ping)
            let pingItem = self.item(with: State.ping.rawValue)
            pingItem?.currentValue = loopResult.ping
            pingItem?.medianValue = self.median(for: State.ping.rawValue)
            
            if self.rmbtClient.isQOSEnabled {
                if let qosTestResultCounters = response.qosTestResultCounters {
                    var total: Int = 0
                    var success: Int = 0
                    for item in qosTestResultCounters {
                        success += item.successCount ?? 0
                        total += item.totalCount ?? 0
                    }
                    
                    loopResult.qosFailed = total - success
                    loopResult.qosPassed = success
                    loopResult.qosPerformed = total
                    
                    let item = self.item(with: State.qos.rawValue)
                    item?.currentValue = "\(success)/\(total)"
                    item?.medianValue = self.median(for: State.qos.rawValue)
                    self.showStateResult()
                    self.currentState = .ping
                }

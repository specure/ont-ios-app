...
class QosMeasurementIndexTableViewController: UITableViewController {
    ...
    private func parseQosResultForIndexView() { // TODO: should be done by rmbt-ios-client library
        if let resultDetail = historyResult?.qosTestResultCounters {
            for i in resultDetail {
                if let type = QosMeasurementType(rawValue: i.testType ?? ""),
                   let successCount = i.successCount,
                   let totalCount = i.totalCount {
                    let item = QosMeasurementItem(type: type, count: totalCount, successCount: successCount, descriptionText: "descriptionText")

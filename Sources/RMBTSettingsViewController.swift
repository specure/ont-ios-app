...
class RMBTSettingsViewController: TopLevelTableViewController {
    ...
    enum Identifier: String {
        ...
        case server = "server"
        case isOverrideServer = "isOverrideServer"
    ...
    let qosOptions = [
        RMBTSelectListItemsViewController.Item(identifier: L("RMBT-SETTINGS-QOSMANUALLY"), value: RMBTSettings.NerdModeQosMode.manually.rawValue),
        RMBTSelectListItemsViewController.Item(identifier: L("RMBT-SETTINGS-QOSNETWORK"), value: RMBTSettings.NerdModeQosMode.newNetwork.rawValue),
        RMBTSelectListItemsViewController.Item(identifier: L("RMBT-SETTINGS-QOSALWAYS"), value: RMBTSettings.NerdModeQosMode.always.rawValue)
    ]
    ...
    override func viewDidLoad() {
        ...
        if RMBTConfiguration.RMBT_VERSION == 3 {
    ...
    func prepareSections() {
        ...
        if RMBTConfiguration.RMBT_VERSION == 3 {
            ...
            about.title = String(format: format, RMBTConfiguration.appName)
...
//        generalSection.items.append(language)
        ...
        if RMBTSettings.sharedSettings.isDevModeEnabled {
            ...
            let isOverrideServer = RMBTSettingsItem()
            isOverrideServer.title = L("RMBT-SETTINGS-IS-OVERRIDE-CONTROL-SERVER")
            isOverrideServer.identifier = .isOverrideServer
            isOverrideServer.mode = .switcher
            
            devSection.items.append(isOverrideServer)
            
            let server = RMBTSettingsItem()
            server.title = L("RMBT-SETTINGS-CONTROL-SERVER")
            server.identifier = .server
            server.mode = .options
            server.value = self.selectedServerOption()
            
            devSection.items.append(server)
    ...
    func isNeedUpdateControlServer(_ okBlock: @escaping () -> Void, cancelBlock: @escaping () -> Void) {
        let alert = UIAlertController(title: "Warning", message: "Your data will be deleted. Continue?", preferredStyle: .alert)
    ...
    static func updateControlServer() {
        let configuration = RMBTConfiguration
        if RMBTSettings.sharedSettings.isOverrideServer,
           let serverIdentifier = RMBTSettings.sharedSettings.serverIdentifier,
           let server = configuration.serverConfigurations.first(where: { $0.identifier == serverIdentifier }) {
            // Override control servers configuration
            RMBTConfig.sharedInstance.configClientIdentifier(server.clientIdentifier)
            RMBTConfig.sharedInstance.configNewCS(server: server.controlServer)
            RMBTConfig.sharedInstance.configNewMeasurementCS(server: server.measurementControlServer)
            RMBTConfig.sharedInstance.configNewCS_IPv4(server: server.ipv4)
            RMBTConfig.sharedInstance.configNewCS_IPv6(server: server.ipv6)
            // Map server
            RMBTConfig.sharedInstance.configNewMapServer(server: server.mapServer)
            RMBTConfig.updateSettings {} error: { _ in }
        } else {
            // Override control servers configuration
            RMBTConfig.sharedInstance.configClientIdentifier(configuration.clientIdentifier)
        ...
        if indexPath.section < self.sections.count {
            ...
            case .switcher:
                ...
                bindSwitch(cell.valueSwitch, toSettingsKeyPath: item.identifier.rawValue) { [weak self] (value) in
                    ...
                    if item.identifier == .isOverrideServer {
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.3, execute: {
                            self?.isNeedUpdateControlServer {
                                settings.isOverrideServer = value
                                self?.prepareSections()
                                self?.tableView.reloadData()
                            } cancelBlock: {
                                self?.prepareSections()
...
//    }
    ...
    func selectedServerOption() -> String? {
        let configuration = RMBTConfiguration
        if RMBTSettings.sharedSettings.value(forKey: "serverIdentifier") == nil,
           let config = configuration.serverConfigurations.first {
            RMBTSettings.sharedSettings.setValue(config.identifier, forKey: "serverIdentifier")
        }
        if let currentValue = RMBTSettings.sharedSettings.value(forKey: "serverIdentifier") as? String {
            for config in configuration.serverConfigurations {
               if config.identifier == currentValue {
...
//                } else
                if item.identifier == .enableQos {
                    ...
                    let identifier = self.selectedQosOption()
                    vc.selectedItem = RMBTSelectListItemsViewController.Item(identifier: identifier, value: currentValue)
                    vc.onDidSelectHandler = { [weak self] value in
                ...
                if item.identifier == .server {
                    let configuration = RMBTConfiguration
                    vc.title = L("RMBT-SETTINGS-ENABLEQOSMEASUMENTS")
                    vc.items = configuration.serverConfigurations.map({ RMBTSelectListItemsViewController.Item(identifier: $0.identifier, value: $0) })
                    if let serverIdentifier = self.selectedServerOption(),
                       let server = configuration.serverConfigurations.first(where: { $0.identifier == serverIdentifier }) {
                        vc.selectedItem = RMBTSelectListItemsViewController.Item(identifier: serverIdentifier, value: server)
                    }
                    
                    vc.onDidSelectHandler = { [weak self] value in
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.3, execute: {
                            self?.isNeedUpdateControlServer {
                                if let currentValue = value as? RMBTConfigurationProtocol.ServerConfig {
                                    RMBTSettings.sharedSettings.setValue(currentValue.identifier, forKey: "serverIdentifier")
                                    self?.prepareSections()
                                    self?.tableView.reloadData()
                                }
                            } cancelBlock: {

...
class RMBTHistoryIndexViewController: TopLevelViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func getNextBatch() {
        ...
        if loading == false && self.state != .serverNotAvailable {
            ...
            MeasurementHistory.sharedMeasurementHistory.getHistoryWithFilters(filters: activeFilters,
                                                                              size: UInt(kBatchSize),
                                                                              page: UInt(nextBatchIndex),
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.row >= testResults.count {
            // Loading cell
            let cell: UITableViewCell! = tableView.dequeueReusableCell(withIdentifier: "RMBTHistoryLoadingCell", for: indexPath)
            ...
            cell.qosResultsLabel.text      = testResult.qosPercentString
            //
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        guard indexPath.row < testResults.count else { return }
        
        let result = testResults[indexPath.row] as RMBTHistoryResult
        if UIDevice.isDeviceTablet() && RMBTConfiguration.RMBT_VERSION > 2 {
...
// MARK: Segues
    ...
    @IBAction func startFirstTest(sender: AnyObject) {
        ...
        if RMBTConfiguration.RMBT_VERSION == 2,
